...
(d)b$\displaystyle  d_{\infty} (\underline{x},\underline{y}=\left(\sum_{i=1}^{n}(x_i-y_i)^p\right)^{1/p}) $

(e) $S(\underline{x},\underline{y})=\begin{cases}
 0& \text{se } \underline{x}=\underline{y}\\
 1& \text{se } \underline{x}\neq \underline{y}
\end{cases}$
